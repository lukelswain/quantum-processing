...
class Hamiltonian:
    ...
    def matrix(self, t):
        ...
        h = h - (constants.hbar)*self.delta_func(t, self.delta_max)*np.eye(4)
        return h 
...
class System:
    def __init__(self, initial_wavefunction, hamiltonian, n_t=400, t_max=1e-6):
        ...
        self.ts_scaled = (self.ts*self.hamiltonian.delta_max)/self.t_max
    ...
    def trotter_evolve(self, wavefunction):
        trotter_evolved = list(np.zeros(len(self.ts_scaled)))
        for i, j in enumerate(self.ts_scaled):
    ...
    def graph_probability(self, states, v_func, v_max, n_v = 50):
        ...
        if self.hamiltonian.delta_func == constant_fn:
            ...
            axs[0].plot((self.ts*10**6), np.array([self.hamiltonian.delta_func(t, self.hamiltonian.delta_max) for t in self.ts_scaled])*10**(-8), color='#ffc000', linewidth=1.5)
            ...
            for i in range(len(normalised_states)):
                for r, v in enumerate(vs):
                    ...
                    axs[i+1].plot((self.ts*10**6), [self.probability(normalised_states[i], trotter_states[r]) for r, t in enumerate(self.ts_scaled)], color = colormap[r])
                self.hamiltonian.v_update(constant_fn, v_max, 0)
                trotter_states = self.trotter_evolve(self.initial_state)
                axs[i+1].plot((self.ts*10**6), [self.probability(normalised_states[i], trotter_states[r]) for r, t in enumerate(self.ts_scaled)], color = "#00852a", linewidth=2)
...
def compare_numerical_analytic(initial_wavefunction, n_v):
    ...
    compare_system = System(initial_wavefunction, Hamiltonian(2*np.pi*10**6, constant_fn, 0, 0, 2*np.pi*10**8))
    vs = np.linspace(0, 35, n_v)
...
def main():
    ...
    # system_1.graph_probability([[1,0,0,0], [0,1,1,0]], v_fn_1, 2*np.pi*10**20)

    system_2 = System([1,0,0,0], h_time_dependent)
    print(system_2.state(system_2.ts[-1]))
    system_2.graph_probability([[1,0,0,0], [0,1,1,0]], v_fn_1, 2*np.pi*10**8)

    # compare_numerical_analytic([1,0,0,0], 40)
